"""
app2.py — Perbandingan Inferensi: SAHI+YOLOv8s-seg vs Baseline YOLOv8s-seg
Sistem Grading: SCAA Green Coffee Classification 5-Level (FAO Annex 7)
Jalankan: streamlit run app2.py
"""
...
# ============================================================
st.set_page_config(
    page_title="Coffee Defect — SCAA 5-Level Grading",
    page_icon="☕",
    layout="wide",
)
...
# ============================================================
# KONSTANTA ATURAN CACAT (divisor untuk konversi ke TDC)
# ============================================================
DEFECT_RULES = {
    ...
    "severe insect" : {"kategori": 1, "divisor": 5},
...
# ============================================================
# SCAA 5-LEVEL GRADING TABLE
# ============================================================
SCAA_GRADES = [
    {
        "num"    : 1,
        "name"   : "Specialty Grade",
        "syarat" : "Cat.1 (Primer) = 0  |  TDC Kat-2 ≤ 5",
        "ket"    : "Tidak ada cacat primer, maks. 5 cacat sekunder dalam 350g",
        "color"  : "#155724",
        "bg"     : "#d4edda",
        "border" : "#28a745",
        "emoji"  : "🏆",
    },
    {
        ...
        "name"   : "Premium Grade",
        "syarat" : "Total TDC ≤ 8",
        "ket"    : "Maks. 8 full defects dalam 350g, boleh ada cacat primer",
        "color"  : "#0c5460",
        "bg"     : "#d1ecf1",
        "border" : "#17a2b8",
        "emoji"  : "🥈",
    },
    {
        "num"    : 3,
        "name"   : "Exchange Grade",
        "syarat" : "Total TDC 9–23",
        "ket"    : "9–23 full defects dalam 350g, bebas dari cup faults",
        "color"  : "#6b5400",
        "bg"     : "#fff3cd",
        "border" : "#ffc107",
        "emoji"  : "🟡",
    },
    {
        "num"    : 4,
        "name"   : "Below Standard",
        "syarat" : "Total TDC 24–86",
        "ket"    : "24–86 defects per 300g, kualitas rata-rata",
        "color"  : "#7d3800",
        "bg"     : "#ffe5d0",
        "border" : "#fd7e14",
        "emoji"  : "🟠",
    },
    {
        ...
        "name"   : "Off Grade",
        "syarat" : "Total TDC > 86",
        "ket"    : "Lebih dari 86 defects, grade terendah yang tersedia",
        "color"  : "#7b1a22",
        "bg"     : "#f8d7da",
        "border" : "#dc3545",
        "emoji"  : "🔴",
...
# SCAA GRADING LOGIC
# ============================================================
def get_scaa_grade(cat1_tdc: int, total_tdc: int) -> dict:
    """Kembalikan dict grade SCAA 5-level berdasarkan TDC."""
    if cat1_tdc == 0 and total_tdc <= 5:
        return SCAA_GRADES[0]   # Grade 1
    elif total_tdc <= 8:
        return SCAA_GRADES[1]   # Grade 2
    elif total_tdc <= 23:
        return SCAA_GRADES[2]   # Grade 3
    elif total_tdc <= 86:
        return SCAA_GRADES[3]   # Grade 4
    else:
        return SCAA_GRADES[4]   # Grade 5


def compute_grade(result: dict) -> dict:
    """Hitung TDC per kategori dan tentukan Grade SCAA."""
    ...
    for defect, cnt in sorted(counts.items()):
        ...
        if key in DEFECT_RULES:
            rule = DEFECT_RULES[key]
            ...
            breakdown.append({
                ...
                "Tipe Cacat" : tipe,
                "Jumlah Biji": cnt,
                "Ekuivalensi": f"1 TDC = {rule['divisor']} biji",
                "TDC"        : tdc,
            })
            if rule["kategori"] == 1:
                cat1_tdc += tdc
    ...
    grade     = get_scaa_grade(cat1_tdc, total_tdc)
...
def render_grade_card(grade_info: dict, model_name: str):
    """Render kartu SCAA 5-level grading + tabel breakdown."""
    import pandas as pd
    g = grade_info["grade"]

    # ── Kartu utama grade ────────────────────────────────────
    st.markdown(f"""
    <div style="background:{g['bg']};border:2px solid {g['border']};
                border-radius:12px;padding:16px;margin-top:8px;">
      <h3 style="margin:0;color:{g['color']};">
        {g['emoji']} Grade {g['num']} — {g['name']}
      </h3>
      <p style="margin:6px 0 2px 0;font-size:0.9em;color:#1a1a1a;">
        <b>Syarat:</b> {g['syarat']}
      </p>
      <p style="margin:0;font-size:0.85em;color:#333;">
        {g['ket']}
      </p>
      <hr style="margin:10px 0;border-color:{g['border']};">
      <p style="margin:0;font-size:0.95em;color:#1a1a1a;">
        <b>Total Deteksi:</b> {grade_info['total_defects']} biji &nbsp;|&nbsp;
        <b>TDC Primer:</b> {grade_info['cat1_tdc']} &nbsp;|&nbsp;
        <b>TDC Sekunder:</b> {grade_info['cat2_tdc']} &nbsp;|&nbsp;
        <b>Total TDC:</b> {grade_info['total_tdc']}
      </p>
    </div>
    """, unsafe_allow_html=True)

    # ── Tabel breakdown per kelas ────────────────────────────
    if grade_info["breakdown"]:
        ...
        st.dataframe(
            df.style.apply(highlight_tipe, axis=1),
...
# ============================================================
# REFERENSI TABEL SCAA (sidebar)
# ============================================================
def render_scaa_reference():
    """Tampilkan tabel referensi grade SCAA di sidebar."""
    st.markdown("### 📖 Referensi Grade SCAA")
    for g in SCAA_GRADES:
        st.markdown(f"""
        <div style="background:{g['bg']};border-left:4px solid {g['border']};
                    border-radius:6px;padding:8px 12px;margin-bottom:6px;">
          <b style="color:{g['color']};">{g['emoji']} Grade {g['num']} — {g['name']}</b><br>
          <span style="font-size:0.8em;color:#333;">{g['syarat']}</span>
        </div>
        """, unsafe_allow_html=True)
...
def run_baseline_inference(image_path: str, model,
    ...
    return {"boxes": boxes, "scores": scores, "cls_ids": cls_ids,
            "masks": masks, "img": img_bgr, "class_names": res.names}


# ============================================================
# INFERENSI SAHI (MANUAL SLICING)
# ============================================================
def slice_image_for_infer(img: np.ndarray, slice_h: int, slice_w: int,
                          overlap: float):
    H, W = img.shape[:2]
    ...
    for y0 in range(0, H, step_y):
        for x0 in range(0, W, step_x):
            patch = img[y0:min(y0+slice_h, H), x0:min(x0+slice_w, W)]
...
def run_sahi_inference(image_path: str, model,
                       ...
                       nms_iou: float = 0.5, device: str = "cpu") -> dict:
    img_orig = cv2.imread(image_path)
    H_orig, W_orig = img_orig.shape[:2]
    all_boxes, all_scores, all_cls_ids, all_masks = [], [], [], []
    last_names = {}

    for patch, x_off, y_off in slice_image_for_infer(img_orig, slice_h, slice_w, overlap):
        with torch.no_grad():
        ...
        for i, box in enumerate(res.boxes):
            ...
            bx1 = max(0, min(bx1 + x_off, W_orig))
            bx2 = max(0, min(bx2 + x_off, W_orig))
            by1 = max(0, min(by1 + y_off, H_orig))
            by2 = max(0, min(by2 + y_off, H_orig))
            all_boxes.append([bx1, by1, bx2, by2])
            all_scores.append(float(box.conf.item()))
            all_cls_ids.append(int(box.cls.item()))
            if res.masks is not None and i < len(res.masks.xy):
                pts = res.masks.xy[i].copy()
                pts[:, 0] += x_off
                pts[:, 1] += y_off
                all_masks.append(pts)
            else:
    ...
    keep     = torchvision.ops.nms(boxes_t, scores_t, nms_iou).tolist()

    return {
        "boxes"      : [all_boxes[k]   for k in keep],
        "scores"     : [all_scores[k]  for k in keep],
        "cls_ids"    : [all_cls_ids[k] for k in keep],
        "masks"      : [all_masks[k]   for k in keep],
        "img"        : img_orig,
...
def render_legend(class_names: dict):
    ...
    for i, (cls_id, name) in enumerate(sorted(class_names.items())):
        ...
        with cols[i % len(cols)]:
            st.markdown(
                f'<span style="background:#{r:02x}{g:02x}{b:02x};'
...
def main():
    # ── Header ──────────────────────────────────────────────
    st.markdown("""
    <h1 style="text-align:center;">☕ Coffee Defect — SCAA 5-Level Grading</h1>
    <p style="text-align:center;color:#666;">
        SAHI + YOLOv8s-seg &nbsp;vs&nbsp; Baseline YOLOv8s-seg
        &nbsp;·&nbsp; SCAA Green Coffee Classification (FAO Annex 7)
    </p>
    <hr>
    """, unsafe_allow_html=True)
    ...
    with col_a:
        if baseline_ok:
            st.success("✅ Baseline model ditemukan")
        else:
            st.error(f"❌ Baseline tidak ditemukan: `{baseline_path}`")
    with col_b:
        if sahi_ok:
            st.success("✅ SAHI model ditemukan")
        else:
    ...
    if run_btn:
        ...
        for file_idx, uploaded_file in enumerate(uploaded):
            ...
            with st.spinner("🔍 Menjalankan Baseline inference..."):
                baseline_result = run_baseline_inference(
                    ...
                    conf=conf_thr, iou=iou_thr, device=device_opt)
            ...
            # ── Legenda warna ─────────────────────────────────
            all_names = {**baseline_result["class_names"],
                         **sahi_result["class_names"]}
            if all_names:
                with st.expander("🎨 Legenda Kelas", expanded=False):
                    render_legend(all_names)

            # ── SCAA Grading ──────────────────────────────────
            st.markdown("#### 📋 SCAA 5-Level Grading")
            baseline_grade = compute_grade(baseline_result)
            sahi_grade     = compute_grade(sahi_result)

            gcol1, gcol2 = st.columns(2)
            with gcol1:
                st.markdown("**🔵 Baseline YOLOv8s-seg**")
                render_grade_card(baseline_grade, "Baseline")
            with gcol2:
                st.markdown("**🟢 SAHI + YOLOv8s-seg**")
                render_grade_card(sahi_grade, "SAHI")

            # ── Ringkasan perbandingan grade ──────────────────
            import pandas as pd
            st.markdown("#### ⚖️ Ringkasan Perbandingan Grade")
            bg  = baseline_grade["grade"]
            sg  = sahi_grade["grade"]
            summary_rows = [
                {
                    "Model"       : "🔵 Baseline",
                    ...
                    "Grade SCAA"  : f"Grade {bg['num']} — {bg['name']}",
                },
                {
                    "Model"       : "🟢 SAHI",
                    ...
                    "Grade SCAA"  : f"Grade {sg['num']} — {sg['name']}",
                },
            ]
            st.dataframe(pd.DataFrame(summary_rows),
                         use_container_width=True, hide_index=True)

            # ── Perbandingan jumlah per kelas ─────────────────
            with st.expander("📊 Perbandingan Jumlah Deteksi per Kelas"):
                ...
                for cls in all_classes:
                    b_cnt = baseline_grade["counts"].get(cls, 0)
                    s_cnt = sahi_grade["counts"].get(cls, 0)
                    rows.append({
                        "Kelas"        : cls,
                        "Baseline"     : b_cnt,
                        "SAHI"         : s_cnt,
                        "Δ (SAHI−Base)": s_cnt - b_cnt,
                    })
                if rows:
        ...
        # Preview gambar sebelum inferensi
